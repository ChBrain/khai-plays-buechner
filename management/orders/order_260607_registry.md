...
Every playhouse must declare a `registry.json` mapping its plays and enforce single-sentence blurb constraints to decouple play card ingestion from legacy markdown parsing. The Theatre Manager (franzos) shall introduce the local registry and align the playbook `## Arc` sections to contain only the short, single-sentence German blurb.

## Orders

1. Update `package.json` to bump the version to `0.9.0` and define export mapping for `registry.json`.
